...
end_generation <- as.integer(args[5])
...
for (param_set in 1:num_param_sets) {
  for (replicate in 1:num_replicates) {
    ...
    # add indexes
    tagged <- raw_data[generation >= start_generation & generation <= end_generation, c("params", "replicate") := list(param_set,replicate) ]
...
#output results
write.table(results, file = sprintf("%s_combined.csv", filename) )